...
const DEFAULT_SETTINGS: BusinessSettings = {
  ...
  receiptHeader: 'Thank you for shopping with us!', // Default header
  receiptFontSize: 12, // Default font size
  receiptWidth: 80, // Default width
  showBusinessName: true, // Show business name by default
  showAddress: true, // Show address by default
  showTIN: true, // Show TIN by default
  showBIRPermit: true, // Show BIR Permit by default
  showContactNumber: true, // Show contact number by default
};
...
console.log('Retrieved Products:', savedProducts);
    ...
    console.log('Parsed Products:', parseProductDates(savedProducts));
  ...
  const adjustStock = (
    ...
    console.log(`Adjusting stock for product: ${productId}, Quantity: ${quantity}, Type: ${type}, Variant ID: ${variantId}`); // Debugging line
    console.log(`Adjusting stock for product: ${productId}, Quantity: ${quantity}, Type: ${type}`); // Debugging line
    console.log(`Adjusting stock for product: ${product.name}, Quantity: ${quantity}, Type: ${type}`); // Debugging line
    if (!product) return;
  ...
  const updateProduct = (productId: string, updates: Partial<Product>): void => {
    const updated = products.map(p => {
      if (p.id === productId) {
        // Calculate total stock from variants if product has variants
        let totalStock = p.stock; // Default to current stock
        if (updates.hasVariants && updates.variants && updates.variants.length > 0) {
          totalStock = updates.variants.reduce((sum, variant) => sum + variant.stock, 0);
        }
        return { ...p, ...updates, stock: totalStock, updatedAt: new Date() };
      }
      return p;
    });
    setProducts(updated);
  ...
  const recordSale = (saleData: Omit<Sale, 'id' | 'receiptNumber' | 'timestamp'>): Sale => {
    ...
    // Use adjustStock to properly handle both variant and non-variant products
    saleData.items.forEach(item => {
      const userId = saleData.cashierId || 'system'; // Use cashier ID or fallback
      if (item.variant && item.variant.id) {
        ...
        // Reduce stock for non-variant products
        adjustStock(
          item.productId,
          item.quantity,
          'remove',
  ...
  const getLowStockProducts = (threshold = 10): Product[] => {
    return products.filter(product => {
      if (product.variants && product.variants.length > 0) {
        ...
        console.log(`Checking low stock for product: ${product.name}, Variant Stocks: ${product.variants.map(v => v.stock)}`);
        return product.variants.some(variant => variant.stock <= threshold && variant.stock > 0);
      }
      // Check main product stock for products without variants
      console.log(`Checking low stock for product: ${product.name}, Stock: ${product.stock}`);
  ...
  const getOutOfStockProducts = (): Product[] => {
    return products.filter(product => {
      if (product.variants && product.variants.length > 0) {
        ...
        console.log(`Checking out of stock for product: ${product.name}, Variant Stocks: ${product.variants.map(v => v.stock)}`);
        const hasStock = product.variants.some(variant => variant.stock > 0); 
        // Check if all variants are out of stock
        return !hasStock && product.variants.every(variant => variant.stock <= 0);
        console.log(`Product: ${product.name}, Out of Stock: ${!hasStock}`);
        return !hasStock;
      }
      // Check main product stock for products without variants
      console.log(`Checking out of stock for product: ${product.name}, Stock: ${product.stock}`);
  ...
  const getVariantStockAlerts = () => {
    const alerts = [];
    console.log('Checking variant stock alerts for all products...');
    for (const product of products) {
      if (product.variants && product.variants.length > 0) {
        console.log(`Checking product: ${product.name}, Variant Stocks: ${product.variants.map(v => v.stock)}`);
        ...
        if (lowStockVariants.length > 0) {
          console.log(`Low stock alert for product: ${product.name}, Variants: ${lowStockVariants.map(v => v.size)}`);
          alerts.push({
            product,
            type: 'low',
        ...
        if (outOfStockVariants.length > 0) {
          console.log(`Out of stock alert for product: ${product.name}, Variants: ${outOfStockVariants.map(v => v.size)}`);
          alerts.push({
            product,
            type: 'out',
    ...
    console.log(`Variant Stock Alerts: ${JSON.stringify(alerts)}`);
    return alerts;
  };
