...
export const ReceiptPage = () => {
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      const businessInfo = `
BALANDZXC POS
123 Barangay Street, Gabao, Irosin
TIN: 123-456-789-000
BIR Permit: FP-12345678
Contact: +63 912 345 6789
`;

      const thankYouMessage = "Salamat sa inyong pagbili!\nThank you for your business!";
        ...
        <html>
          <head>
            ...
            <style>
              body { 
                ...
                margin: 20px;
                max-width: 300px;
                text-align: center; /* Centering the entire body content */
              }
              pre { 
          ...
          <body>
            <div style="text-align: center;">
              <h1>Receipt #${sale.receiptNumber}</h1>
              <p>Transaction completed successfully</p>
              <p>${new Date(sale.timestamp).toLocaleDateString()}</p>
              <p>${new Date(sale.timestamp).toLocaleTimeString()}</p>
              <pre>${businessInfo}</pre>
            </div>
            <pre>${receiptText}</pre>
            <div style="text-align: center;">
              <pre>${thankYouMessage}</pre>
            </div>
            <script>
...
<div className="bg-[hsl(var(--primary))] text-white p-6 text-center">
  <h1 className="text-2xl font-bold">ISOGOOD'S DINER</h1>
<p className="text-sm">123 Barangay Street, Gabao, Irosin</p>
